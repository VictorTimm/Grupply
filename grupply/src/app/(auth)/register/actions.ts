...
export async function registerAction(formData: FormData) {
  ...
  const { error: adminPreflightError } = await admin.from("organizations").select("id").limit(1);
  if (adminPreflightError) {
    logRegistrationEvent("error", "admin_preflight_failed", {
      requestId,
      flow: values.flow,
      code: adminPreflightError.code ?? null,
      message: adminPreflightError.message,
    });
    redirect(registerErrorPath(mapAdminRuntimeError(adminPreflightError), values.flow));
  }
